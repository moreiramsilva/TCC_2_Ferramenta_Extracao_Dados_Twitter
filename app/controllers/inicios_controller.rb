...
class IniciosController < ApplicationController
	...
	def exibir
		@item = params[:arg]
		@tweet_dbs = TweetDb.where(tag_associada: @item).sort({"data":-1})
	end
	...
	def busca
		...
		mnr.nuvem_de_palavras
		mnr.mineracao_apriori

		#FAVORITOS 
		@favoritos = TweetDb.where(tag_associada: @parametro).sort({"favoritos":-1}).limit(1)
		#MAIS RETWEETADOS
		@retweetados = TweetDb.where(tag_associada: @parametro).sort({"retweet":-1}).limit(1)
...
class Mineracao
	...
	def initialize(argum , argum2, argum3, argum4)
		@argum = argum
		@argum2 = argum2
		@argum3 = argum3
		@argum4 = argum4
	...
	def analise_de_padrao_e_sentimentos
		...
		if (@argum3 == "atual")
			busca = TweetDb.where(tag_associada: @argum).where(hora_extracao: @argum2).limit(@argum4)
		else
			busca = TweetDb.where(tag_associada: @argum).limit(@argum4)
		end
